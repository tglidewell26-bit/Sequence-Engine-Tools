...
## Overview
A two-step AI-powered scientific outreach sequence generator for Bruker Spatial Biology. Pastes a lead data row from Google Sheets, researches the company via Perplexity Sonar, generates a tailored 6-part outreach sequence via OpenAI GPT-4o, then applies deterministic post-processing (link injection, availability insertion, asset selection and placement).
...
- **AI Step 2**: OpenAI GPT-4o via user's own API key (sequence generation)
...
3. OpenAI GPT-4o generates 6-part outreach sequence using research brief + lead intel
...
server/services/
  ...
  sequence-generator.ts   - OpenAI GPT-4o sequence generation + text parser
...
- `OPENAI_API_KEY` - OpenAI API key for GPT-4o sequence generation
